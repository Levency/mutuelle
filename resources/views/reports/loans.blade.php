...
<div class="content">
    <div class="summary">
        Capital total : <span>{{ \App\Models\Setting::get('currency', 'Gourdes') }} {{ number_format($total_principal, 2) }}</span> &nbsp;|&nbsp;
        Encours actifs : <span style="color:#dc2626">{{ \App\Models\Setting::get('currency', 'Gourdes') }} {{ number_format($total_remaining, 2) }}</span> &nbsp;|&nbsp;
    ...
    <table>
        <thead>
            <tr>
                ...
                <th>Taux %</th>
                <th>Durée</th>
                <th>Total dû {{ \App\Models\Setting::get('currency', 'Gourdes') }}</th>
                <th>Restant {{ \App\Models\Setting::get('currency', 'Gourdes') }}</th>
                <th>Statut</th>
                <th>Décaissement</th>
            </tr>
        </thead>
        <tbody>
            ...
            <tr>
                ...
                <td style="text-align:right">{{ \App\Models\Setting::get('currency', 'Gourdes') }} {{ number_format($loan->principal_amount, 2) }}</td>
                <td style="text-align:center">{{ $loan->interest_rate }}%</td>
                <td style="text-align:center">{{ $loan->term_months }} mois</td>
                <td style="text-align:right; font-weight:bold">{{ \App\Models\Setting::get('currency', 'Gourdes') }} {{ number_format($loan->total_to_repay, 2) }}</td>
                <td style="text-align:right; color:{{ $loan->balance_remaining > 0 ? '#dc2626' : '#059669' }}">{{ \App\Models\Setting::get('currency', 'Gourdes') }} {{ number_format($loan->balance_remaining, 2) }}</td>
                <td>
