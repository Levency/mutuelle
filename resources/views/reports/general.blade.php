...
<div class="content">
    ...
    <div class="kpi-grid">
        <div class="kpi-box"><div class="val success">{{ \App\Models\Setting::get('currency', 'Gourdes') }} {{ number_format($balance, 2) }}</div><div class="lbl">Solde Global</div></div>
        <div class="kpi-box"><div class="val">{{ \App\Models\Setting::get('currency', 'Gourdes') }} {{ number_format($available, 2) }}</div><div class="lbl">Disponible</div></div>
        <div class="kpi-box"><div class="val warning">{{ \App\Models\Setting::get('currency', 'Gourdes') }} {{ number_format($solidarity, 2) }}</div><div class="lbl">Solidarité</div></div>
        <div class="kpi-box"><div class="val danger">{{ \App\Models\Setting::get('currency', 'Gourdes') }} {{ number_format($total_loaned, 2) }}</div><div class="lbl">Encours Prêts</div></div>
    </div>
    <div class="kpi-grid">
        <div class="kpi-box"><div class="val">{{ $total_members }}</div><div class="lbl">Membres</div></div>
        <div class="kpi-box"><div class="val success">{{ \App\Models\Setting::get('currency', 'Gourdes') }} {{ number_format($total_contributions, 2) }}</div><div class="lbl">Total Cotisé</div></div>
    ...
    <table style="width: 60%;">
        <tr><td>Solde Brut en Caisse</td><td class="text-right font-bold">{{ \App\Models\Setting::get('currency', 'Gourdes') }} {{ number_format($audit['gross_balance'], 2) }}</td></tr>
        <tr><td>Moins : Prêts Actifs (Encours)</td><td class="text-right danger">- {{ \App\Models\Setting::get('currency', 'Gourdes') }} {{ number_format($audit['loans_encumbrance'], 2) }}</td></tr>
        <tr class="bg-gray"><td class="font-bold">SOLDE DISPONIBLE RÉEL</td><td class="text-right font-bold success">{{ \App\Models\Setting::get('currency', 'Gourdes') }} {{ number_format($audit['available_raw'], 2) }}</td></tr>
        <tr><td>Dont part réservée Solidarité</td><td class="text-right warning">{{ \App\Models\Setting::get('currency', 'Gourdes') }} {{ number_format($audit['solidarity'], 2) }}</td></tr>
    </table>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="text-right">{{ \App\Models\Setting::get('currency', 'Gourdes') }} {{ number_format($m->contributions_sum_amount ?? 0, 2) }}</td>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th>N° Membre</th>
                <th>N° Reçu</th>
                <th class="text-right">Montant {{ \App\Models\Setting::get('currency', 'Gourdes') }}</th>
                <th>Statut</th>
            </tr>
        </thead>
        <tbody>
            ...
            <tr>
                ...
                <td class="text-right font-bold">{{ \App\Models\Setting::get('currency', 'Gourdes') }} {{ number_format($c->amount, 2) }}</td>
    ...
    <table>
        <thead>
            <tr>
                <th>Membre</th>
                <th>Capital {{ \App\Models\Setting::get('currency', 'Gourdes') }}</th>
                <th>Durée</th>
                <th>Total dû</th>
                <th>Restant</th>
                <th>Statut</th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ \App\Models\Setting::get('currency', 'Gourdes') }} {{ number_format($loan->principal_amount, 2) }}</td>
                <td>{{ $loan->term_months }} mois</td>
                <td class="font-bold">{{ \App\Models\Setting::get('currency', 'Gourdes') }} {{ number_format($loan->total_to_repay, 2) }}</td>
                <td class="text-right danger">{{ \App\Models\Setting::get('currency', 'Gourdes') }} {{ number_format($loan->balance_remaining, 2) }}</td>
                <td>{{ $loan->status }}</td>
            </tr>
            @empty
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>#{{ $r->loan_id }}</td>
                <td class="text-right font-bold success">{{ \App\Models\Setting::get('currency', 'Gourdes') }} {{ number_format($r->amount_paid, 2) }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
